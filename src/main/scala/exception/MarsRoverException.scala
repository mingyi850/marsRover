...
object MarsRoverException {
  ...
  def invalidMissionString(mission: String) = s"Invalid mission: $mission is not a valid mission"

  def invalidCoordinateString(x: String, y: String) = s"Invalid coordinate: $x, $y is not a valid coordinate"
  ...
  case class MissionValidationError(mission: String) extends Exception(invalidMissionString(mission))
  case class InputStructureValidationError() extends Exception("Invalid input structure")
}
