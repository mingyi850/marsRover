  ...
  "ActiveMarsRover" should {
    "Return correct new state when performing rotation" in {
      ...
      ActiveMarsRover(position, East)
        .execute(RotateLeft)
        .shouldEqual(ActiveMarsRover(position, North))
      ActiveMarsRover(position, South)
        .execute(RotateRight)
        .shouldEqual(ActiveMarsRover(position, West))
    }
    "Return correct new position when movingForward" in {
      ...
      ActiveMarsRover(position, East)
        .execute(MoveForward)
        .shouldEqual(ActiveMarsRover(Coordinate(3, 3), East))
      ActiveMarsRover(position, South)
        .execute(MoveForward)
        .shouldEqual(ActiveMarsRover(Coordinate(2, 2), South))
    }
    "Become LostMarsRover if leaving grid boundaries" in {
      implicit val grid: Grid = Grid(9, 9)
      ActiveMarsRover(Coordinate(9, 9), North)
        .execute(MoveForward)
        .shouldEqual(LostMarsRover(Coordinate(9, 9), North))
      ActiveMarsRover(Coordinate(0, 1), West)
        .execute(MoveForward)
        .shouldEqual(LostMarsRover(Coordinate(0, 1), West))
    }
  }
