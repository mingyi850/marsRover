...
class InputParserTest extends AnyWordSpec with Matchers {
  ...
  case class TestInputParser() extends InputParser {
    override def parse(): Try[List[RoverMission]] = Success(
      List.empty[RoverMission]
    )
  }
  ...
  "InputParser" should {
    ...
    "InputParser.parseLines" should {
      "Split input into head and tail" in {
        ...
        inputParser
          .parseLines(List("abc", "def", "cde"))
          .shouldEqual(Success("abc", List("def", "cde")))
      }
    ...
    "InputParser.parseMissions" should {
      "Return List of RoverMissions if input is valid" in {
        ...
        inputParser
          .parseMissions(
            List("(2, 3, E) LFRFF", "(0, 2, N) FFLFRFF"),
            Grid(4, 8)
          )
          .shouldEqual(
            Success(
              List(
                RoverMission(
                  MarsRover(Coordinate(2, 3), Orientation.East)(Grid(4, 8)),
                  List(
                    RotateLeft,
                    MoveForward,
                    RotateRight,
                ...
                RoverMission(
                  MarsRover(Coordinate(0, 2), Orientation.North)(Grid(4, 8)),
                  List(
                    MoveForward,
                    MoveForward,
                    RotateLeft,
      ...
      "Return Failure if any input is invalid" in {
        ...
        inputParser
          .parseMissions(
            List("(2, 3, E) LFRFF", "(0, 2, N) FFLFRFFG"),
            Grid(4, 8)
          )
          .isFailure
          .shouldEqual(true)
        inputParser
          .parseMissions(
            List("(2, 3, A) LFRFF", "(0, 2, N) FFLFRFF"),
            Grid(4, 8)
          )
          .isFailure
          .shouldEqual(true)
    ...
    "InputParser.parseMissionString" should {
      "Return RoverMission if input is valid" in {
        val inputParser = TestInputParser()
        inputParser
          .parseMissionString("(2, 3, E) LFRFF", Grid(4, 8))
          .shouldEqual(
            Success(
              RoverMission(
                MarsRover(Coordinate(2, 3), Orientation.East)(Grid(4, 8)),
      ...
      "Return Failure if orientation is invalid" in {
        val inputParser = TestInputParser()
        inputParser
          .parseMissionString("(2, 3, D) LFRFF", Grid(4, 8))
          .isFailure
          .shouldEqual(true)
        inputParser
          .parseMissionString("(2, 3, 3) LFRFF", Grid(4, 8))
          .isFailure
          .shouldEqual(true)
      }
      ...
      "Return Failure if coordinates are malformed" in {
        ...
        inputParser
          ...
          .shouldEqual(true)
        inputParser
          .parseMissionString("a, 3, E LFRFF", Grid(4, 8))
          .isFailure
          .shouldEqual(true)
      }
      ...
      "Return Failure if commands are invalid" in {
        val inputParser = TestInputParser()
        inputParser
          .parseMissionString("(2, 3, E) LFRFFG", Grid(4, 8))
          .isFailure
          .shouldEqual(true)
        inputParser
          .parseMissionString("(2, 3, E) LFRFF  ", Grid(4, 8))
          ...
          .shouldEqual(true)
        inputParser
          .parseMissionString("(2, 3, E) LFRFF  F", Grid(4, 8))
          .isFailure
          .shouldEqual(true)
      }
    }
